...
export class Bililogin extends plugin {
    ...
    async bililogin(e) {
        ...
        if (!fs.existsSync(tempBilibiliDir)) {
            fs.mkdirSync(tempBilibiliDir, { recursive: true });
        }
    ...
    async refreshUserToken(e) {
        ...
        for (const [DedeUserID, userCookies] of Object.entries(cookies)) {
            ...
            const url = `${signApi}/refresh?accesskey=${userCookies.access_token}&refresh_token=${userCookies.refresh_token}`;
            const headers = {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    refreshedAccounts.push(`账号 ${DedeUserID} 未知错误`);
                    anyFailure = true;
                }
            } catch (error) {
